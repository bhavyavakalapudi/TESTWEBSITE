...
import { batchJobSchema, type InsertBatchJob } from "@/lib/schemas";
import { removeAuthToken, getAuthToken, simulateStartBatch } from "@/lib/auth";
...
export default function DashboardPage() {
  ...
  const batchMutation = useMutation({
    mutationFn: async (data: InsertBatchJob) => {
      return await simulateStartBatch(data);
    },
    onSuccess: (data) => {
      if (data.success) {
        toast({
        ...
        toast({
          title: "Error",
          description: data.message || "Failed to start batch",
          variant: "destructive",
        });
      }
    },
    onError: () => {
      toast({
        title: "Error",
        description: "An error occurred while starting the batch",
        variant: "destructive",
      });
    },
  });
  ...
  return (
    <div
      ...
      style={{
        backgroundImage:
          "url('https://media.istockphoto.com/id/1448034091/photo/defocused-lobby.jpg?b=1&s=612x612&w=0&k=20&c=CLpN9aKMGFY0M60ZJ6iUMy_VJC2W9mwDYckOaYGSr7g=')",
      }}
    >
